    ...
    if (useWireframe) {
      ...
      glBegin(GL_TRIANGLES);
        for (int i=0; i<f.size(); i++){
            ...
            (f[i].vt[0])->p.SendAsTextureCoord();
            (f[i].v[0])->p.SendAsVertex();

            (f[i].v[1])->n.SendAsNormal();
            (f[i].vt[1])->p.SendAsTextureCoord();
            (f[i].v[1])->p.SendAsVertex();

            (f[i].v[2])->n.SendAsNormal();
            (f[i].vt[2])->p.SendAsTextureCoord();
            (f[i].v[2])->p.SendAsVertex();
      }
      glEnd();
      if(id>0){
...
//printf("dopo FirstPass nv=%d nvt=%d nf=%d nt=%d\n",nv,nvt,nf,nt);
  ...
  while(fscanf(file, "%s", buf) != EOF) {
      ...
      case 'v':               // v, vn, vt
            ...
            case '\0':          // vertex
                ...
                break;
            case 't':          // vertex
                fscanf(file, "%f %f", &xt, &yt);
                vt[nvt].p = Point3( xt,yt,0 );
                nvt++;
                break;
            default:
                break;
     ...
     case 'f':               // face
           ...
           if (strstr(buf, "//")) {
              ...
              while(fscanf(file, "%d//%d", &vc, &nc) > 0) {
                ...
                //ta++; tb++; tc++;
                Face newface( &(v[va]), &(v[vc]), &(v[vb]) ); // invoco il costruttore di faccia
                //newface.set_Texture(&(vt[ta]), &(vt[tb]), &(vt[tc]));
                newface.set_Texture( &(vt[ta]),  &(vt[tc]), &(vt[tb]) );
                f.push_back( newface ); // inserico la nuova faccia in coda al vettore facce
                nt++;
                vb=vc;
                tb=tc;
                while(fscanf(file, "%d/%d/%d", &vc, &tc, &nc) > 0) {
                   ...
                   //newface.set_Texture(&(vt[ta]), &(vt[tb]), &(vt[tc]) );
                   newface.set_Texture(&(vt[ta]), &(vt[tc]), &(vt[tb]) );
                   //newface.set_Texture(&(vt[tc]), &(vt[tb]), &(vt[ta]) );
                   //newface.set_Texture(&(vt[tc]), &(vt[ta]), &(vt[tb]) );
                   //newface.set_Texture(&(vt[tb]), &(vt[ta]), &(vt[tc]) );
                   //newface.set_Texture(&(vt[tb]), &(vt[tc]), &(vt[ta]) );
                   f.push_back( newface ); // inserico la nuova faccia in coda al vettore facce
                   nt++;
                   vb=vc;
                }
